...
class StoreBallInBucketController extends Controller
{
    public function fill_bucket(Request $request)
    {
        ...
        $filteredData = collect($collectData)->except('_token');
        $hasValue = $filteredData->filter()->isNotEmpty();
        if ($hasValue) {
            $errorMsg = '';
            $request_number = rand();
            ...
            return redirect()->route('home')->with(['errorMsg' => 'Please enter number of balls.']);
        }
        /**
         * 
         * Checking if data is stored or not 
         */
        $showSuccessMessage = StoreBallInBucket::where(['request_number' => $request_number])->count();
        $showErrorMessage = RemainingBall::where(['request_number' => $request_number])->count();

        if($showSuccessMessage > 0 && $showErrorMessage > 0){
            $msg = 'Some balls filled into the bucket but rest of balls are not stored in the bucket please check the result.';
            return redirect()->route('home')->with(['fillbucketsuccess' => $msg]);
        } else if($showSuccessMessage > 0 && $showErrorMessage == 0){
            return redirect()->route('home')->with(['fillbucketsuccess' => 'Ball filled into the bucket successfully.']);
        } else if($showSuccessMessage == 0 && $showErrorMessage > 0){
            $errorMsg = 'Bucket riched maximum possible volume of the balls to be placed so its not allowing any placement.';
            return redirect()->route('home')->with(['errorMsg' => $errorMsg]);
        } else if($showSuccessMessage == 0 && $showErrorMessage == 0){
            return redirect()->route('home')->with(['fillBucketError' => 'Bucket are full can\'t store balls.']);
        }
    }
}
